...
static struct asmop asmop_a, asmop_b, asmop_c, asmop_d, asmop_e, asmop_h, asmop_l, asmop_iyh, asmop_iyl, asmop_hl, asmop_de, asmop_bc, asmop_iy, asmop_jk, asmop_dehl, asmop_hlde, asmop_hlbc, asmop_debc, asmop_bcde, asmop_jkhl, asmop_ahl, asmop_aiy, asmop_zero, asmop_one, asmop_mone;
...
static void // TODO: This function shall be fully replaced by cost3 in the future.
...
cost3 (int z80_bytes /* also most other ports */, int tlcs90_bytes, int tlcs870_bytes, int tlcs870c_bytes,
  float z80_states /* also z80n */, float z180_states, float r4k_clocks, float r6k_clocks, float sm83_cycles, float tlcs90_states, float tlcs870_cycles, float tlcs870c_cycles, float tlcs870c1_cycles, float ez80_cycles, float r800_cycles)
{
  int bytes = -1;
  float states = -1.0;
  if (IS_TLCS90)
  ...
  switch (op1type)
        ...
        case AOP_REG:
          ...
          if (op1->aopu.aop_reg[offset1]->rIdx == IYL_IDX || op1->aopu.aop_reg[offset1]->rIdx == IYH_IDX ||
            op2->aopu.aop_reg[offset2]->rIdx == IYL_IDX || op2->aopu.aop_reg[offset2]->rIdx == IYH_IDX)
            {
              ...
              if (count)
                cost3 (2, -1, -1, -1, 8, -1, -1, -1, -1, -1, -1, -1, -1, 2, 2);
              return (2);
            }
        case AOP_DUMMY:
          if (op1->aopu.aop_reg[offset1]->rIdx == A_IDX || op1type == AOP_DUMMY || op2->aopu.aop_reg[offset2]->rIdx == A_IDX || op2type == AOP_DUMMY)
            {
              if (count)
                cost3 (1, 1, 1, 1, 4, 4, 2, 2, 4, 2, 1, 1, 1, 1, 1); // ld a, r / ld r, a
              return (1);
            }
          else
            {
              if (count)
                cost3 (1, 2, 2, 2, 4, 4, 2, 2, 4, 4, 2, 1, 2, 1, 1); // ld r, r'
              return (1 + IS_TLCS);
            }
        case AOP_IMMD:
        case AOP_LIT:
          if (op1->aopu.aop_reg[offset1]->rIdx == IYL_IDX || op1->aopu.aop_reg[offset1]->rIdx == IYH_IDX)
            {
              wassert (HAS_IYL_INST);
              if (count)
                cost3 (3, -1, -1, -1, 11, -1, -1, -1, -1, -1, -1, -1, -1, 2, 3); // ld ir, #n
              return (3);
            }
          else
            {
              if (count)
                cost3 (2, 2, 2, 2, 7, 6, 4, 4, 8, 4, 2, 2, 2, 2, 2); // ld r, #n
              return (2);
            }
        case AOP_SFR:
          if (count)
            {
              if (IS_SM83)
                ...
                cost3 (2, -1, -1, -1, 11, 9, -1, -1, -1, -1, -1, -1, -1, 3, 3); // in a, (n)
              if (!aopInReg (op1, 0, A_IDX) && op1type != AOP_DUMMY)
                cost3 (1, 1, 1, 1, 4, 4, 2, 2, 4, 2, 1, 1, 1, 1, 1); // ld r, a
            }
          return ((aopInReg (op1, 0, A_IDX) || op1type == AOP_DUMMY) ? 2 : 3);
        case AOP_STK:
          if (count)
            cost3 (3, 3, -1, 3, 19, 14, 9, 10, -1, 10, -1, 5, 5, 4, 5); // ld r, d(ix)
          return (3);
          ...
          if (count)
            {
              ...
              if (aopInReg (op1, 0, A_IDX))
                cost3 (1, 2, 1, 1, 7, 6, 5, 5, 8, 6, 2, 2, 2, 2, 2); // ld a, (hl)
              else
                cost3 (1, 2, 2, 2, 7, 6, 5, 5, 8, 6, 3, 4, 3, 2, 2); // ld r, (hl)
            }
          return (4);
        case AOP_IY:
          if (count)
            {
              cost3 (4, 3, -1, 3, 14, 12, 8, 8, 0, 6, -1, 3, 3, 4, 4); // ld iy, #nn
              cost3 (3, 3, 0, 3, 19, 14, 9, 10, 0, 10, 0, 5, 5, 4, 5); // ld r, d(iy)
            }
          return (7);
        case AOP_PAIRPTR:
          ...
          if (op2->aopu.aop_pairId == PAIR_BC || op2->aopu.aop_pairId == PAIR_DE)
            {
              if (count)
                {
                  cost3 (1, 2, -1, -1, 7, 6, 6, 6, 8, 6, -1, -1, -1, 2, 2); // ld a, (rr)
                  if (!aopInReg (op1, 0, A_IDX))
                    cost3 (1, 1, 1, 1, 4, 4, 2, 2, 4, 2, 1, 1, 1, 1, 1); // ld r, a
                }
      ...
      if (count)
        {
          if (IS_SM83)
            ...
            cost3 (2, -1, -1, -1, 11, 10, -1, -1, -1, -1, -1, -1, -1, 3, 3); // out (n), a
        }
      if (aopInReg (op1, 0, A_IDX))
        return (2);
      else
    ...
    case AOP_STK:
      switch (op2type)
        ...
        case AOP_LIT:
          if (count)
            cost3 (4, 4, -1, 4, 19, 15, 11, 12, -1, 12, -1, 6, 5, 5, 5); // ld d(ix), n
          return (4);
        case AOP_SFR:          /* 2 from in a, (...) */
          if (count)
            {
              if (IS_SM83)
                ...
                cost3 (2, -1, -1, -1, 11, 9, -1, -1, -1, -1, -1, -1, -1, 3, 3); // in a, (n)
              cost3 (3, 3, -1, 3, 19, 15, 10, 11, -1, 10, -1, 5, 4, 4, 5); // ld d(ix), a
            }
          return (5);
        case AOP_STK:
          if (count)
            {
              cost3 (3, 3, -1, 3, 19, 4, 9, 10, -1, 10, -1, 5, 5, 4, 5); // ld a, d(ix)
              cost3 (3, 3, -1, 3, 19, 15, 10, 11, -1, 10, -1, 5, 4, 4, 5); // ld d(ix), a
            }
          return (6);
        case AOP_EXSTK: // Approximation. Don't really know if this is really exstk at this point, anyway.
          ...
          if (count)
            {
              ...
              cost3 (3, 3, -1, 3, 19, 4, 9, 10, -1, 10, -1, 5, 5, 4, 5); // ld a, d(iy)
              cost3 (3, 3, -1, 3, 19, 15, 10, 11, -1, 10, -1, 5, 4, 4, 5); // ld d(ix), a
            }
          return (10);
        case AOP_PAIRPTR:
      ...
      switch (op2type)
        ...
        case AOP_DUMMY:
          if (count)
            {
              if (aopInReg (op1, 0, A_IDX))
                cost3 (1, 2, 1, 1, 7, 7, 6, 6, 8, 6, 2, 2, 2, 2, 2); // ld (hl), a
              else
                cost3 (1, 2, 2, 2, 7, 7, 6, 6, 8, 6, 3, 3, 3, 2, 2); // ld (hl), r
            }
          return (4);
        case AOP_IMMD:
        case AOP_LIT:
          if (count)
            cost3 (2, 3, 2, 2, 10, 9, 7, 7, 12, 8, 3, 4, 3, 3, 3); // ld (hl), n
          return (5 + IS_TLCS90);
        case AOP_STK:
          if (count)
            {
      ...
      switch (op2type)
        ...
        case AOP_REG:
          if (op1->aopu.aop_pairId == PAIR_HL)
            ...
            {
              if (count)
                cost3 (3, 3, -1, 3, 19, 15, 10, 11, -1, 10, -1, 5, 4, 4, 5); // ld d(ix), r
              return (3);
            }
          else
            wassert (0);
...
op8_cost (const asmop *op, int offset)
{
  switch (op->type)
    ...
    case AOP_DUMMY:
      cost3 (1, 2, 2, 2, 4, 4, 2, 2, 4, 4, 2, 2, 2, 1, 1);
      return;
    case AOP_IMMD:
    case AOP_LIT:
      cost3 (2, 2, 2, 2, 7, 6, 4, 4, 8, 4, 2, 2, 2, 2, 2);
      return;
    case AOP_STK:
...
bit8_cost (const asmop *op)
{
  switch (op->type)
    ...
    case AOP_HL:
      cost3 (3, 3, 3, 3, 10, 9, 6, 6, 12, 6, 3, 3, 3, 3, 3); // ld hl, #nn
      cost3 (2, 2, -1, -1, 15, 6, 10, 10, 16, 8, -1, -1, -1, 5, 5);
      return;
    case AOP_IY:
      cost3 (4, 3, -1, 3, 14, 12, 8, 8, -1, 6, -1, 3, 3, 4, 4); // ld iy, #nn
      cost3 (4, 3, -1, -1, 23, 19, 13, 14, -1, 12, -1, -1, -1, 7, 7);
...
emit3Cost (enum asminst inst, const asmop *op1, int offset1, const asmop *op2, int offset2)
{
  ...
  switch (inst)
    ...
    case A_CCF:
    case A_SCF:
      cost3 (1, 1, 1, 1, 4, 3, 2, 2, 4, 2, 1, 1, 1, 2, 1);
    case A_CPL:
    case A_RLA:
    case A_RLCA:
    case A_RRA:
    case A_RRCA:
      cost3 (1, 1, 1, -1, 4, 3, 2, 2, 4, 2, 1, -1, -1, 1, 1);
      return;
    case A_NEG:
      cost3 (2, 1, -1, -1, 8, 6, 4, 13, 2, 2, -1, -1, 2, 2, 2);
      return;
    case A_RLD:
    case A_RRD:
      cost3 (2, 2, 2, 2, 18, 16, -1, -1, -1, 12, 7, 7, 8, 5, 5);
      return;
    case A_LD:
    ...
    case A_LDI:
      ...
      cost3 (2, 2, -1, -1, 16, 12, 10, 10, -1, 14, -1, -1, -1, 5, 4);
      return;
    case A_LDF:
    ...
    case A_XOR:
      if (IS_TLCS && !aopInReg (op1, offset1, A_IDX) && (op2->type == AOP_LIT || op2->type == AOP_IMMD))
        {
          if (op1->type == AOP_REG)
            cost3 (-1, 3, 3, 3, -1, -1, -1, -1, -1, 6, 3, 3, 3, -1, -1);
    ...
    case A_INC:
      ...
      return;
    case A_RET:
      cost3 (1, 1, 1, 1, 10, 9, 8, 8, 16, 10, 6, 6, 6, 5, 3);
      return;
    case A_RETI:
      cost3 (2, 1, 1, 1, 14, 12, 12, 12, 16, 14, 6, 6, 6, 6, 5);
      return;
    case A_RETN:
      cost3 (2, -1, 2, 2, 14, 12, -1, -1, -1, -1, 7, 7, 7, 6, 5);
      return;
    case A_RL:
    case A_RLC:
    case A_RR:
    ...
    case A_SWAP:
      ...
      if (aopInReg (op1, offset1, A_IDX) && IS_TLCS870)
        ...
        cost3 (2, -1, 2, 2, 8, -1, -1, -1, 8, -1, 4, 4, 7, -1, -1);
      return;
    default:
  ...
  switch (inst)
    ...
    case A_DEC:
      ...
      if (aopInReg (op1, offset1, IY_IDX))
        cost3 (2, 1, -1, 1, 10, 7, 4, 4, -1, 4, -1, 2, 2, 2, 2);
      else
        cost3 (1, 1, 1,	1, 6, 4, 2, 2, 8, 4, 2, 2, 2, 1, 1);
      return;
    case A_ADD:
      if (aopInReg (op1, offset1, IY_IDX) && op2->type == AOP_REG)
        cost3 (2, 2, -1, 2, 15, 10, 4, 4, -1, 8, -1, 4, 3, 2, 2);
      else if (op2->type == AOP_LIT || op2->type == AOP_IMMD)
        cost3 (-1, 3, -1, 4, -1, -1, -1, -1, -1, 6, -1, 4, 4, -1, -1);
      else
        cost3 (1, 2, -1, 2, 11, 7, 2, 2, 8, 8, -1, 4, 3, 1, 1);
      return;
    case A_ADC:
    case A_SBC:
      cost3 (2, 2, -1, 2, 15, 10, 4, 4, 0, 8, -1, 4, 3, 2, 2);
      return;
    case A_CP:
      if (op2->type == AOP_LIT || op2->type == AOP_IMMD)
        cost3 (3, 3, -1, 4, -1, -1, 6, 6, -1, 6, -1, 4, 4, -1, -1);
      else if (op2->type == AOP_STK)
        cost3 (3, 3, -1, 3, -1, -1, -1, 12, -1, 12, -1, 7, 6, -1, -1);
      else
        cost3 (2, 2, -1, 2, -1, -1, 4, 4, -1, 8, -1, 4, 3, -1, -1);
      return;
    case A_AND:
    case A_OR:
      if (IS_RAB && aopInReg (op1, offset1, IY_IDX))
        ...
        cost3 (1, 2, -1, 2, -1, -1, 2, 2, -1, 8, -1, 4, 3, -1, -1);   
      return;
    case A_EX:
      if (aopInReg (op1, offset1, DE_IDX))
        cost3 (1, 1, 2, 2, 4, 3, 2, 2, -1, 2, 3, 3, 3, 1, 1);
    ...
    case A_POP:
      wassert (!op2);
      if (aopInReg (op1, offset1, IY_IDX))
        cost3 (2, 1, -1, 2, 14, 12, 9, 9, -1, 10, -1, 5, 5, 4, 5);
      else
        cost3 (1, 1, 2, 1, 10, 9, 7, 7, 12, 10, 5, 4, 4, 3, 4);
      return;
    case A_PUSH:
      wassert (!op2);
      if (op1->type == AOP_LIT || op1->type == AOP_IMMD)
        {
          ...
          cost3 (4, -1, -1, -1, 23, -1, 15, 16, -1, -1, -1, -1, -1, -1, -1);
        }
      else if (aopInReg (op1, offset1, IY_IDX))
        cost3 (2, 1, -1, 2, 15, 13, 12, 13, -1, 8, -1, 4, 4, 4, 5);
      else
        cost3 (1, 1, 2, 1, 11, 11, 10, 11, 16, 8, 4, 3, 3, 3, 4);
      return;
    case A_RL:
      wassert (IS_RAB);
    ...
    case PAIR_HL:
      if (srcPair == PAIR_IX || srcPair == PAIR_IY)
      ...
      else
        {
          ...
          cost3 (1, 2, 2, 2, 4, 4, 2, 2, 4, 4, 2, 1, 2, 1, 1);
  ...
  while (size)
    {
      if ((size >= 2 && aop->type == AOP_LIT || aop->type == AOP_IMMD) &&
        ...
        {
          emit2 ("push bcde");
          cost3 (2, -1, -1, -1, -1, -1, 18, 19, -1, -1, -1, -1, -1, -1, -1);
  ...
  else
    {
      ...
      /* Todo: Use even cheaper ex hl, (sp) and ex iy, (sp) when possible. */
        ...
        (aop->size >= 2 || pairId != PAIR_IY && optimize.allow_unsafe_read))
        {
          ...
          if (pairId == PAIR_HL)
            cost3 (3, 4, -1, 4, 16, 15, 11, 11, -1, 12, -1, 6, 6, 5, 5);
          else
            cost3 (4, 4, -1, 4, 20, 18, 13, 13, -1, 12, -1, 6, 6, 6, 6);
          ...
          switch (aop->size - offset)
            ...
            default:
              ...
              if (IS_RAB)
                ...
                {
                  emit2 ("ld hl, !*hl");
                  cost3 (2, 2, 2, 2, -1, -1, -1, -1, -1, 8, 4, 4, 4, 4, -1);
                }
              else
                {
                  ...
                  if (IS_SM83)
                    ...
                    {
                      cost3 (1, 2, 1, 1, 7, 6, 5, 5, 8, 6, 2, 2, 2, 2, 2); // ld a, (hl)
                      cost3 (1, 1, 1, 1, 6, 4, 2, 2, 8, 4, 2, 2, 2, 1, 1); // inc hl
                    }
                  emit2 ("ld h, !*hl");
                  cost3 (1, 2, 2, 2, 7, 6, 5, 5, 8, 6, 3, 4, 3, 2, 2);
  ...
  else
    {
      ...
      switch (aop->type)
        ...
        case AOP_DIR:
          ...
          cost3 (3, 4, -1, 4, 13, 12, 9, 9, 16, 10, -1, 5, 5, 4, 4);
          ...
          else
            {
              ...
              if (aop->paged)
              ...
              else
                {
                  ...
                  cost3 (2, -1, -1, -1, 11, 9, -1, -1, -1, -1, -1, -1, -1, 3, 3);
                }
        ...
        case AOP_EXSTK:
          if (!IY_RESERVED && !IS_SM83 && !IS_TLCS870)
            {
              setupPair (PAIR_IY, aop, offset);
              dbuf_tprintf (&dbuf, "!*iyx", offset);
              break;
...
poppairwithsavedreg (PAIR_ID pair, short survivingreg, short tempreg)
{
  ...
  cost3 (1, 2, 2, 2, 7, 7, 6, 6, 8, 6, 3, 3, 3, 2, 2);
  _pop (PAIR_HL);
  _pop (PAIR_AF);
  _pop (pair);
}
...
cheapMove (asmop *to, int to_offset, asmop *from, int from_offset, bool a_dead)
{
  ...
  if (to->type == AOP_REG && from_index && !to_index && - _G.stack.pushed - _G.stack.offset >= -128 && !_G.omitFramePtr)
   ...
   else if (to_index && !from_index && from->type == AOP_REG && - _G.stack.pushed - _G.stack.offset >= -128 && !_G.omitFramePtr)
    {
      ...
      cost3 (3, 3, -1, 3, 19, 15, 10, 11, -1, 10, -1, 5, 4, 4, 5);
      _pop(PAIR_IY);
      return;
    }
  ...
  if (from_index && !to_index && !aopInReg (to, to_offset, L_IDX) && !aopInReg (to, to_offset, H_IDX))
    {
      _push (PAIR_IY);
      emit2 ("ex (sp), hl");
      cost3 (1 + IS_RAB, 2, -1, 3, 19, 16, 15, 15, -1, 14, -1, 8, 8, 5, 5);
  ...
  if (from->type == AOP_IY && aopInReg (to, to_offset, A_IDX) && from_offset < from->size)
  ...
  else if (aopInReg (from, from_offset, A_IDX) && to->type == AOP_IY)
    {
      ...
      cost3 (3, 4, -1, 4, 13, 13, 10, 10, 16, 10, -1, 5, 5, 4, 4);
    }
...
commitPair (asmop *aop, PAIR_ID id, const iCode *ic, bool dont_destroy) // Obsolete. Replace uses by genMove or genMove_o.
{
  ...
  if (!regalloc_dry_run && !IS_SM83 && (aop->type == AOP_STK || aop->type == AOP_EXSTK) && !sp_offset
      && ((!IS_RAB && id == PAIR_HL) || id == PAIR_IY) && !dont_destroy)
    {
      ...
      if (id == PAIR_IY)
        cost3 (2, 2, -1, 3, 23, 19, 15, 15, -1, 14, -1, 8, 8, 6, 6);
      else
        cost3 (1 + IS_RAB, 2, -1, 3, 19, 16, 15, 15, -1, 14, -1, 8, 8, 5, 5);
      spillPair (id);
    }
      ...
      if (abs (sp_offset) <= 127)
        {
          ...
          if (id == PAIR_HL || IS_TLCS90)
            cost3 (2, 3, -1, 3, -1, -1, 11, 12, -1, 12, -1, 5, 5, -1, -1);
          else
            cost3 (3, 3, -1, 3, -1, -1, 13, 14, -1, 12, -1, 5, 5, -1, -1);
        }
      else
        {
          emit2 ("ld %d (ix), hl", fp_offset);    /* Relative to frame pointer. */
          cost3 (2 + IS_EZ80, 3, -1, 3, -1, -1, 11, 12, -1, 12, -1, 6, 5, 5, -1);
        }
    }
      ...
      if ((aop->type == AOP_IY || aop->type == AOP_HL) && !IS_SM83 && aop->size == 2)
        {
          ...
          if (id == PAIR_HL)
            cost3 (3, 4, -1, 4, 16, 16, 13, 13, -1, 12, -1, 6, 6, 5, 5);
          else
            cost3 (4, 4, -1, 4, 20, 19, 15, 15, -1, 12, -1, 6, 6, 6, 6);
        }
      else
        {
          switch (id)
  ...
  for (int j = 0; j < n;)
    {
      ...
      if ((IS_RAB || IS_EZ80 || IS_TLCS90 || IS_TLCS870C || IS_TLCS870C1) && i + 1 < n && !assigned[i + 1] && hl_free &&
        ...
        (source->type == AOP_STK && source_fp_offset >= -128 && source_fp_offset <= 127 || source_sp))
        {
          ...
          cost3 (2 + IS_EZ80, 3, -1, 3, -1, -1, 9, 10, -1, 12, -1, 6, 6, 5, -1);
          if (result_sp)
...
/*-----------------------------------------------------------------*/
static void
genCopy (asmop *result, int roffset, asmop *source, int soffset, int sizex, bool a_dead, bool hl_dead, bool de_dead, bool jk_dead)
{
  ...
  for (int i = 0; i < n && source->type == AOP_REG;)
    {
      ...
      if (!IS_SM83 && !IS_TLCS870 && !IS_RAB && !(IS_TLCS90 && optimize.codeSpeed) && // The sm83 doesn't have ex (sp), hl. The Rabbits and tlcs90 have it, but ld 0 (sp), hl is faster. For the Rabbits, they are also the same size.
        ...
        aopInReg (source, soffset + i, HL_IDX) && hl_free)
        {
          ...
          emit3w (A_EX, ASMOP_DE, ASMOP_HL);
          spillPair (PAIR_HL);
          genCopy (result, roffset + i, ASMOP_DE, 0, 2, a_free, true, true, false);
          if (!de_free)
            _pop (PAIR_DE);
          assigned[i] = true;
          assigned[i + 1] = true;
...
genMove_o (asmop *result, int roffset, asmop *source, int soffset, int size, bool a_dead_global, bool hl_dead_global, bool de_dead_global, bool iy_dead_global, bool f_dead)
{
  ...
  emitDebug ("; genMove_o size %d result type %d source type %d hl_dead %d de_dead %d", size, result->type, source->type, hl_dead_global, de_dead_global);
  ...
  if ((result->type == AOP_REG || result->type == AOP_STK || result->type == AOP_EXSTK) && (source->type == AOP_REG || source->type == AOP_STK || source->type == AOP_EXSTK))
    {
      ...
      genCopy (result, roffset, source, soffset, csize, a_dead_global, hl_dead_global, de_dead_global, jk_dead_global);
      roffset += csize;
      size -= csize;
...
adjustStack (int n, bool af_free, bool bc_free, bool de_free, bool hl_free, bool iy_free)
{
  ...
  if (IS_TLCS90 && abs(n) > (optimize.codeSize ? 2 + (bc_free || de_free || hl_free || iy_free || n < 0) * 2 : 1))
    ...
    hl_free)
    {
      ...
      cost3 (1, 2, -1, 2, 11, 7, 2, 2, 8, 8, -1, 4, 3, 1, 1);
      emit2 ("ld sp, hl");
      cost3 (1, 1, 2, 2, 6, 4, 2, 2, 8, 4, 3, 2, 2, 1, 1);
      spillPair (PAIR_HL);
    ...
    iy_free)
    {
      ...
      cost3 (2, 2, -1, 2, 15, 10, 4, 4, -1, 8, -1, 4, 3, 2, 2);
      emit2 ("ld sp, iy");
      cost3 (2, 1, -1, 2, 10, 7, 2, 4, -1, 4, -1, 2, 2, 2, 2);
      spillPair (PAIR_IY);
      n -= n;
    }
  ...
  if (IS_FLOAT (type))
    {
      ...
      cost3 (2, 2, 2, 2, 8, 6, 4, 4, 8, 4, 3, 3, 3, 2, 2);
      skipbyte = size - 1;
    }
  while (size--)
    if (size != skipbyte)
  ...
  for(int i = 0; i < size;)
    {
      if (i + 1 < size && isRegDead (BC_IDX, ic))
        ...
        {
          emit2 ("ld a, !*hl");
          cost3 (1, 2, 1, 1, 7, 6, 5, 5, 8, 6, 2, 2, 2, 2, 2);
          emit2 ("push af");
          cost3 (1, 1, 2, 1, 11, 11, 10, 11, 16, 8, 4, 3, 3, 3, 4);
          emit2 ("inc sp");
          cost3 (1, 1, 1, 1, 6, 4, 2, 2, 8, 4, 2, 2, 2, 1, 1);
      ...
      if (!jump && !IS_TLCS90 && !IS_RAB && // TLCS-90 doesn't have rst. Rabbit rst has different semantics.
        ...
        aopIsLitVal (ic->left->aop, 0, 1, 0x20) || aopIsLitVal (ic->left->aop, 0, 1, 0x28) || aopIsLitVal (ic->left->aop, 0, 1, 0x30) || aopIsLitVal (ic->left->aop, 0, 1, 0x38)))
        {
          ...
          if (!regalloc_dry_run)
            ...
            cost3 (1, -1, -1, -1, 11, 11, 8, 11, 16, -1, -1, -1, -1, 5, 4);
        }
...
genEndFunction (iCode *ic)
{
  ...
  if (poststackadjust && // Try to merge both stack adjustments.
    ...
    !IFFUNC_ISISR (sym->type) && !IFFUNC_ISCRITICAL (sym->type))
    {
      ...
      if (hl_free)
        cost3 (2, 3, -1, 3, -1, -1, 9, 10, -1, 12, -1, 6, 6, -1, -1);
      else
        cost3 (3, 3, -1, 3, -1, -1, 11, 12, -1, 12, -1, 6, 6, -1, -1);
...
genRet (const iCode *ic)
{
      ...
      do
        {
          ...
          cost3 (2, 3, 2, 2, 10, 9, 7, 7, 12, 8, 3, 4, 3, 3, 3);
          lit >>= 8;
          if (size > 1)
            emit3w (A_INC, ASMOP_HL, 0);
        }
  ...
  // gbz80 doesn't have have ldir. Rabbit 2000 to Rabbit 3000 (i.e. r2k and r2ka port) have an ldir wait state bug that affects copies between different types of memory.
    ...
    !IS_SM83 && !IS_R2K && !IS_R2KA && (ic->left->aop->type == AOP_DIR || ic->left->aop->type == AOP_IY))
    {
      ...
      cost3 (3, 3, 3, 3, 10, 9, 6, 6, 12, 6, 3, 3, 3, 3, 3);
      emit2 ("ldir");
      cost3 (2, 2, -1, -1, 21 * size -5, 14 * size - 2, 7 * size - 1, 7* size - 1, -1, 18 * size - 4, -1, -1, -1, 2 * size - 1, 4 * size);
      ...
      do
        {
          ...
          emit2 ("ld !mems, a", "bc");
          cost3 (1, 2, -1, -1, 7, 7, 7, 7, 8, 6, -1, -1, -1, 2, 2);
          if (size > 1)
  ...
  while (size)
    {
      ...
      if ((IS_R6K_NOTYET || IS_TLCS90 || IS_TLCS870C || IS_TLCS870C1) && size >= 2 && aopInReg (result, offset, HL_IDX) && aopOnStack (right, offset, 2) &&
...
genMultOneChar (const iCode * ic)
{
  ...
  if ((IS_Z180 || IS_EZ80 || IS_Z80N) && ic->result->aop->type == AOP_REG)
    {
      if (!IS_Z80N && (resultsize > 1 ? result->aopu.aop_reg[1]->rIdx == B_IDX : isRegDead (B_IDX, ic))
          && result->aopu.aop_reg[0]->rIdx == C_IDX)
        {
          ...
          emit2 ("mlt bc");
          cost3 (2, -1, 2, 2, 8, 17, -1, -1, -1, -1, 8, 8, 13, 6, -1);
          return;
        }
  ...
  if (IS_Z180 || IS_EZ80)
  ...
  else
    {
      ...
      cost3 (2, 2, -1, -1, 12.375f, 8.75f, 5.0f, 6.0f, -1.0f, 10.0f, -1.0f, -1.0f, -1.0f, 3.75f, 2.0f);
      regalloc_dry_run_state_scale = 1.0f;
    }
      ...
      while (sizel)
        {
          ...
          /* Non-destructive and when exactly one bit per byte is set. */
              ...
              left->aop->type == AOP_REG && !aopInReg (left->aop, offset, IYL_IDX) && !aopInReg (left->aop, offset, IYH_IDX)))
            {
              ...
              if (left->aop->type == AOP_REG)
                ...
                cost3 (2, 2, 2, 2, 12, 9, 7, 7, 16, 6, 3, 3, 3, 3, 3);
              else
                cost3 (4, 3, -1, 3, 20, 15, 10, 11, -1, 10, -1, 5, 5, 5 , 5);
              if (requiresHL (left->aop) && left->aop->type != AOP_REG)
                _pop (PAIR_HL);
          ...
          /* Z180 has non-destructive and. */
          else if ((IS_Z180 || IS_EZ80 || IS_Z80N) && aopInReg (left->aop, 0, A_IDX) && !isRegDead (A_IDX, ic) && bytelit != 0x0ff)
            {
              ...
              cost3 (3, -1, -1, -1, 11, 9, -1, -1, -1, -1, -1, -1, -1, 3, -1);
              sizel--;
              offset++;
            }
          ...
          if (isLiteralBit (~bytelit & 0xffu) >= 0 && aopSame (result->aop, i, left->aop, i, 1) &&
            (result->aop->type == AOP_STK || result->aop->type == AOP_DIR || result->aop->type == AOP_REG && !aopInReg (result->aop, i, IYL_IDX) && !aopInReg (result->aop, i, IYH_IDX)))
            {
              ...
              if (result->aop->type == AOP_STK)
                cost3 (4, 3, -1, 3, 23, 19, 13, 14, -1, 14, -1, 6, 6, 5, 7); // res b, d(ix)
              else if (result->aop->type == AOP_DIR || result->aop->type == AOP_EXSTK)
                cost3 (2, 2, 2, 2, 15, 13, 10, 10, 16, 10, 4, 4, 4, 3, 5); // res b, (hl)
              else
                cost3 (2, 2, 2, 2, 8, 6, 4, 4, 8, 4, 3, 3, 3, 2, 2);
      ...
      if (right->aop->type == AOP_LIT)
        {
          ...
          if (isLiteralBit (bytelit) >= 0 && aopSame (result->aop, i, left->aop, i, 1) &&
            (result->aop->type == AOP_STK || result->aop->type == AOP_DIR || result->aop->type == AOP_REG && !aopInReg (result->aop, i, IYL_IDX) && !aopInReg (result->aop, i, IYH_IDX)))
            {
              ...
              if (result->aop->type == AOP_STK)
                cost3 (4, 3, -1, 3, 23, 19, 13, 14, -1, 14, -1, 6, 6, 5, 7); // set b, d(ix)
              else if (result->aop->type == AOP_DIR || result->aop->type == AOP_EXSTK)
                cost3 (2, 2, 2, 2, 15, 13, 10, 10, 16, 10, 4, 4, 4, 3, 5); // set b, (hl)
              else
                cost3 (2, 2, 2, 2, 8, 6, 4, 4, 8, 4, 3, 3, 3, 2, 2); // set b, r
              if (aopInReg (result->aop, i, A_IDX))
                a_free = false;
              i++;
              continue;
      ...
      if (use_b)
        {
          ...
          cost3 (2, 2, -1, -1, 13, 9, 5, 6, -1, 10, -1, -1, -1, 4, 2); // Assume jump taken.
        }
      else
        {
          emit3 (A_DEC, ASMOP_A, 0);
...
genRightShift (const iCode * ic)
{
  ...
  if (shift_by_zero)
    ...
    !right->aop->valinfo.anything && (right->aop->valinfo.knownbitsmask & 0x7) == 0x7 && !(right->aop->valinfo.knownbits & 0x7))
    {
      ...
      cost3 (2, 2, -1, -1, 8, 7, 4, 4, 8, 4, -1, -1, -1, 2, 2);
      emit2 ("srl %s", regsZ80[countreg].name);
      cost3 (2, 2, -1, -1, 8, 7, 4, 4, 8, 4, -1, -1, -1, 2, 2);
      emit2 ("srl %s", regsZ80[countreg].name);
      cost3 (2, 2, -1, -1, 8, 7, 4, 4, 8, 4, -1, -1, -1, 2, 2);
  ...
  if (!from_far && getPairId (left->aop) == PAIR_IY && !bit_field && rightval_in_range)
    {
      ...
      if (IS_RAB && getPairId_o (result->aop, 0) == PAIR_HL)
      ...
      else if ((IS_EZ80 || IS_TLCS90 || IS_TLCS870C || IS_TLCS870C1) && getPairId_o (result->aop, 0) != PAIR_INVALID)
        {
          ...
          cost3 (3, 3, -1, 3, -1, -1, -1, -1, -1, 12, -1, 6, 6, 5, -1);
          offset = 2;
          size -= 2;
        }
      ...
      while (size--)
        {
          ...
          cost3 (3, 3, -1, 3, 19, 14, 9, 10, -1, 10, -1, 5, 5, 4, 5); // Assume ld r, d(iy)
          offset++;
        }
...
genPackBits (PAIR_ID pair, operand *right, int roffset, int blen, int bstr, PAIR_ID extrapair)
{
  ...
  if (right->aop->type == AOP_LIT && blen == 1 && (pair == PAIR_HL || pair == PAIR_IX || pair == PAIR_IY))
  ...
  else
    {
      ...
      cost3 (1, 2, 2, 3, 4, 4, 2, 2, 4, 4, 2, 3, 3, 1, 1);
      emit2 ("ld !mems, a", _pairs[pair].name);
      regalloc_dry_run_cost += (pair == PAIR_IX || pair == PAIR_IY) ? 3 : 1;
      if (needPopExtra)
        _pop (extrapair);
          ...
          if (canAssignToPtr3 (right->aop))
            {
              ...
              if (right->aop->type == AOP_LIT)
                cost3 (4, 4, -1, 4, 19, 15, 11, 12, -1, 12, -1, 6, 5, 5, 5); // ld d (iy), n
              else
                cost3 (3, 3, -1, 3, 19, 15, 10, 11, -1, 10, -1, 5, 4, 4, 5); // ld d (iy), r
            }
          else
            {
              cheapMove (ASMOP_A, 0, right->aop, offset, true);
      ...
      while (offset < size)
        {
          ...
          if (IS_EZ80 && offset + 1 < size && getPairId_o (right->aop, offset) != PAIR_INVALID)
          ...
          else
            {
              ...
              cost3 (1, 2, 1, 1, 7, 7, 6, 6, 8, 6, 2, 2, 2, 2, 2);
              offset++;
            }
      ...
      if (sym->onStack)
      ...
      else if (aopInReg (ic->result->aop, 2, A_IDX) || aopInReg (ic->result->aop, 2, B_IDX) || aopInReg (ic->result->aop, 2, C_IDX) || aopInReg (ic->result->aop, 2, D_IDX) || aopInReg (ic->result->aop, 2, E_IDX) || aopInReg (ic->result->aop, 2, H_IDX) || aopInReg (ic->result->aop, 2, L_IDX) || HAS_IYL_INST && (aopInReg (ic->result->aop, 2, IYH_IDX) || aopInReg (ic->result->aop, 2, IYL_IDX)))
        {
          ...
          if(aopInReg (ic->result->aop, 2, IYH_IDX) || aopInReg (ic->result->aop, 2, IYL_IDX))
            cost3 (3, -1, -1, -1, 11, -1, -1, -1, -1, -1, -1, -1, -1, 2, 3);
          else
            cost3 (2, 2, 2, 2, 7, 6, 4, 4, 8, 4, 2, 2, 2, 2, 2);
        }
      else
